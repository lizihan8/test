...
int main(int argc, char **argv)
{
	...
	*/
	...
	//搞掉%20之类的东西, 如:文件名:my_[xdight].txt 在URL中 [ 需要被转换成%5b
    strdecode(file, file);      
    ...
    //测试有无请求文件，stat为获取指定目录位置文件file函数，sb为传出参数。
    if (stat(file, &sb) < 0)
...
do_file:
		...
		//发送http协议头，200表示成功, OK是随便写的, 也可以写successful
 ...
 */
static void strdecode(char *to, char *from)         //"%5bfdk13" ----> "[fdk13"
{
    for ( ; *from != '\0'; ++to, ++from) {
...
//strencode(encoded_name, sizeof(encoded_name), name);
static void strencode(char* to, size_t tosize, const char* from)        //"[xi13" ---> "%5bxi13"
{
    int tolen;
